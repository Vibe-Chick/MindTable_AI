...
백엔드는 서버가 아니다. **`out/` 의 정적 JSON 파일이 API다.**
파이프라인을 돌리면 이 파일들이 갱신된다. fetch 해서 그리면 된다.

샘플: **`out/sample/`** (익명화됨). 이 구조 그대로 실데이터가 들어온다.

---

## out/sample/groups.json
...
`out/baseline_r1.json` 은 **랜덤 편성** 결과다. 알고리즘 편성과 나란히
