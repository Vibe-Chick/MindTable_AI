...
## 4. 카드 생성 병렬화(max_workers=8) 실측 — **완료 (2026-09-19)**

통합 세션에는 유효한 키가 없어 미완료로 남겼던 항목. 이후 실제 게이트웨이로
측정했다.

측정 방법 (`test_429.py`):

- 40명 / 그룹 10개. demo 의 8명은 그룹 2개뿐이라 워커 8을 포화시키지 못한다
  (`generate_mock_users` 는 `sample_pool[:n]` 이라 8명이 상한).
- 매 실행 유니크한 user_id 로 캐시를 우회. 캐시 히트 시 HTTP 요청 자체가
  나가지 않아 429 가 영원히 관측되지 않는다.
- `llm_client._post` 를 감싸 실제 HTTP 상태코드를 계측. 재시도로 조용히
  성공해도 429 횟수가 잡힌다.

결과 (claude-haiku-4-5, 국민대 게이트웨이):

| 항목 | 값 |
|---|---|
| 최대 동시 요청 | **8** (워커 포화 확인 — 측정이 유효함) |
| 429 | **0회** |
| 5xx / 기타 4xx | 0회 / 0회 |
| HTTP 요청 | 13회 (카드 10 + 검증 재작성 3) |
| 총 소요 | 12.0초 |

**결론: `MAX_LLM_WORKERS = 8` 유지.** `matching.py` 의 관련 주석도 실측
결과로 교체했다(이전 주석은 확인했다고 적혀 있었으나 사실이 아니었다).

부수 관찰: 카드 10개 중 **3개가 타겟/구체성 검증에 걸려 재작성**됐고
(30%), 그중 1개는 재작성 후에도 검증을 통과하지 못했다. 검증 로직이
실제로 동작하고 있다는 뜻이며, 동시에 카드 프롬프트에 아직 개선 여지가
있다는 신호다.
